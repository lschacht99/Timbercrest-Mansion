  ...
  (function () {
    ...
    function propertyOptionsHTML() {
      return D.PROPERTIES.map((p) => `
        <button class="tcsearch-opt${state.ids.includes(p.id) ? " on" : ""}" data-id="${esc(p.id)}">
          ${esc(p.name)}${p.guests ? ` · sleeps ${p.guests}` : ""}
        </button>
      `).join("");
    }
      ...
      return `
        <div class="tcsearch-panel" role="dialog" aria-label="Search">
          <div class="tcsearch-pane" data-pane="where">
            <h4>Which mansion?</h4>
            <div class="tcsearch-opts" data-property-options>
              ${propertyOptionsHTML()}
            </div>
          </div>
    ...
    if (hero) {
      ...
      const heroSegs = bar.querySelectorAll(".tcsearch-seg");
      const vals = bar.querySelectorAll(".tcsearch-seg .val");

      heroSegs.forEach((seg, i) => {
    ...
    function renderWhereOptions() {
      const opts = wrap.querySelector("[data-property-options]");
      if (!opts) return;
      opts.innerHTML = propertyOptionsHTML();
    }
    ...
    function goToBooking() {
      ...
      if (state.pets) params.set("pets", String(state.pets));
    ...
    document.addEventListener("tc:properties-ready", () => {
